...
int main( void )
{
    ...
    Demo_Enter_Stop_Mode(POWER_SAVING_INTERVAL, GPIOC);
    ...
    while( 1 )
    {
            ...
            if( isMaster == true )
            {
                if( BufferSize > 0 )
                {
                    if( strncmp( ( const char* )Buffer, ( const char* )PongMsg, 4 ) == 0 )
                    {
                        ...
                        PingCount++;
                        UartPrint("Master PING - Count (%d)  RSSI (%d)  SNR (%d)\n",
                                PingCount, RssiValue, SnrValue);
                    }
            ...
            else
            {
                if( BufferSize > 0 )
                {
                    if( strncmp( ( const char* )Buffer, ( const char* )PingMsg, 4 ) == 0 )
                    {
                        ...
                        Radio.Send( Buffer, BufferSize );
                        PongCount++;
                        UartPrint("Slave PONG - Count (%d)  RSSI (%d)  SNR (%d)\n",
                                PongCount, RssiValue, SnrValue);
                    }
        ...
        case RX_ERROR:
            if( isMaster == true )
            {
                ...
                DelayMs( 1 ); 
                Radio.Send( Buffer, BufferSize );
                UartPrint("Master PING - Count (%d)\n", ++PingCount);
            }
            else
            {
                UartPrint("Slave PONG - Count (%d)\n", ++PongCount);
                Radio.Rx( RX_TIMEOUT_VALUE );
            }
            State = LOWPOWER;
            break;
